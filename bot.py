...
admin_state = {}  # admin_id -> {"action": ..., "step": ..., "data": ...}
...
def handle_admin_input(chat_id, text):
    ...
    if action == "broadcast":
        ...
        if uid in users_data:
            ...
            save_json(USERS_FILE, users_data)
            name = users_data[uid].get("first_name", uid)
            send(chat_id, f"🔴 کاربر *{name}* (`{uid}`) بلاک شد.", reply_markup=admin_menu())
        else:
        ...
        if uid in users_data:
            ...
            save_json(USERS_FILE, users_data)
            name = users_data[uid].get("first_name", uid)
            send(chat_id, f"🟢 کاربر *{name}* (`{uid}`) آنبلاک شد.", reply_markup=admin_menu())
        else:
            send(chat_id, "❌ کاربر پیدا نشد.", reply_markup=admin_menu())
        admin_state.pop(ADMIN_ID, None)
        return True
...
def handle_callback(callback):
    ...
    if data == "admin_stats":
        ...
        for uid, info in list(users_data.items())[:20]:
            ...
            msg += (f"{status} *{info.get('first_name', 'بدون نام')}*"
                    f" | `{uid}`"
                    f" | 💬{info.get('message_count', 0)}"
                    f" | 📅{info.get('joined', '-')}\n")
        ...
        send(chat_id, "🔴 آیدی عددی کاربری که میخوای بلاک کنی رو بفرست:", reply_markup={"inline_keyboard": [[{"text": "❌ لغو", "callback_data": "admin_cancel"}]]})

    elif data == "admin_unblock":
        admin_state[ADMIN_ID] = {"action": "unblock"}
        send(chat_id, "🟢 آیدی عددی کاربری که میخوای آنبلاک کنی رو بفرست:", reply_markup={"inline_keyboard": [[{"text": "❌ لغو", "callback_data": "admin_cancel"}]]})

    elif data == "admin_clearmem":
        admin_state[ADMIN_ID] = {"action": "clearmem"}
        send(chat_id, "🗑️ آیدی عددی کاربری که میخوای حافظه‌اش پاک بشه رو بفرست:", reply_markup={"inline_keyboard": [[{"text": "❌ لغو", "callback_data": "admin_cancel"}]]})
...
def handle_message(msg):
    ...
    # ── اگه ادمین در حالت انتظار بود ──
    if user_id == ADMIN_ID and ADMIN_ID in admin_state:
        handled = handle_admin_input(chat_id, text)
        if handled:
            return
